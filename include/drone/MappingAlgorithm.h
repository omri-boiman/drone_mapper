...
private:
    ...
    GridCell3D WorldToGrid(Centi x, Centi y, Centi z) const;

    // Convert grid indices back to world position (cm)
    void GridToWorld(const GridCell3D& cell, Centi& x, Centi& y, Centi& z) const;
    ...
    bool RotateToFace(Centi targetX, Centi targetY);

    // Handle height changes (positive = up, negative = down)
    bool AdjustHeight(Centi targetHeight);
};
